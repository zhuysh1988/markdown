...
if [[ $(ss -lntp |grep -c 8879) -eq 0 ]];then nohub helm serve & ;fi 


kubectl create namespace ceph
kubectl create -f /root/ceph-helm/ceph/rbac.yaml

## label node 
kubectl label node node1.example.com ceph-osd=enabled ceph-osd-device-dev-sdb=enabled ceph-osd-device-dev-sdc=enabled ceph-mon=enabled ceph-mgr=enabled

kubectl label node master3.example.com ceph-osd=enabled ceph-osd-device-dev-sdb=enabled ceph-osd-device-dev-sdc=enabled ceph-mon=enabled ceph-mgr=enabled


## unset label for node
kubectl label node node1.example.com ceph-osd- ceph-osd-device-dev-sdb- ceph-osd-device-dev-sdc- ceph-mon- ceph-mgr-

kubectl label node master3.example.com ceph-osd- ceph-osd-device-dev-sdb- ceph-osd-device-dev-sdc- ceph-mon- ceph-mgr-

## 
helm install --name=ceph local/ceph --namespace=ceph -f /opt/deploy-helm-ceph-jhr/ceph-overrides.yaml
